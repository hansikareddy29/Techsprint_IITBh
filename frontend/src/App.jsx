...
import axios from 'axios';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { Activity, RefreshCw, Zap, ShieldAlert, Cpu, ShieldCheck, Waves, Fingerprint, Sun, Moon } from 'lucide-react';
...
export default function App() {
  ...
  // 1. AUTO-DETECT DEVICE (Only shows the user's device based on IP)
  useEffect(() => {
    const detectDevice = async () => {
      try {
        const res = await axios.get(`${API_BASE}/logs/get-my-device`);
    ...
    detectDevice();
    // Re-check for device every 10 seconds if none is found yet
    const interval = setInterval(() => { if(!deviceId) detectDevice(); }, 10000);
    return () => clearInterval(interval);
  }, [deviceId]);
    ...
    return (
      <div className={`flex items-center justify-center min-h-screen ${isDark ? 'bg-[#0a0a0c]' : 'bg-[#6366F1]'}`}>
        <div className="text-center">
          ...
          <h2 className="text-xl font-black text-white tracking-widest uppercase italic">Establishing Neural Link...</h2>
          <p className="text-white/50 text-[10px] mt-2 font-bold uppercase tracking-widest">Ensure Python Agent is Running</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen w-full flex flex-col items-center py-10 px-4 transition-colors duration-700 font-sans relative overflow-x-hidden ${isDark ? 'bg-[#0a0a0c] text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <div className="w-full max-w-6xl space-y-8 relative z-10">
        ...
        {/* HEADER */}
        <header className={`flex justify-between items-center p-6 rounded-[2.5rem] border backdrop-blur-2xl shadow-2xl
          ${isDark ? 'bg-white/5 border-white/10 shadow-black/40' : 'bg-white border-slate-200 shadow-xl shadow-slate-100'}`}>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
              <h1 className="text-2xl font-black tracking-tighter bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] bg-clip-text text-transparent uppercase italic leading-none">VoltGuard.AI</h1>
              <div className="flex items-center gap-2 mt-1">
                <span className={`h-2 w-2 rounded-full animate-pulse ${deviceId ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
          ...
          <div className="flex items-center gap-4">
            <button onClick={toggleTheme} className={`p-3 rounded-full border transition-all hover:scale-105 active:scale-95
               ${isDark ? 'bg-white/5 border-white/10 text-white' : 'bg-slate-100 border-slate-200 text-indigo-600 shadow-sm'}`}>
            ...
            <div className={`hidden md:flex items-center gap-4 p-2.5 px-6 rounded-2xl border ${isDark ? 'bg-black/20 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
              <Fingerprint size={16} className="text-[#6366F1]"/>
              <div className="flex flex-col text-right">
                <span className="text-[8px] font-black opacity-30 uppercase tracking-widest leading-none">System ID</span>
                <span className="font-bold text-xs tracking-tight uppercase mt-1 max-w-[150px] truncate">{deviceId || "Unknown"}</span>
              </div>
            </div>
          </div>
        </header>
        ...
        {!deviceId ? (
          /* EMPTY STATE: If no device is detected */
          <div className={`p-20 rounded-[3.5rem] border text-center ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-slate-100'}`}>
            ...
            <h2 className="text-2xl font-black uppercase italic tracking-widest mb-2">No Local Agent Found</h2>
            <p className="text-sm opacity-50 font-bold max-w-md mx-auto">Please run the <code className="bg-indigo-500/20 px-2 py-1 rounded">VoltGuard_Agent.py</code> on your laptop to start streaming telemetry.</p>
          </div>
        ) : (
          <>
            {/* METRICS ROW */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <div key={i} className={`p-8 rounded-[3rem] border transition-all h-48 flex flex-col justify-between
                  ${isDark ? 'bg-white/5 border-white/10 shadow-2xl' : 'bg-white border-slate-100 shadow-xl'}`}>
                  <div className="flex justify-between items-start">
                    <span className="text-[10px] font-black opacity-40 uppercase tracking-widest leading-none">{item.label}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
              <div className={`lg:col-span-8 p-10 rounded-[3.5rem] border flex flex-col min-h-[480px]
                ...
                <div className="flex justify-between items-center mb-10">
                  ...
                  <span className="text-[10px] font-black px-5 py-2 rounded-full border border-indigo-500/20 text-indigo-500">LIVE FEED</span>
                </div>
                <div className="flex-grow w-full min-h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={history}>
                      ...
                      <XAxis dataKey="timestamp" hide />
                      <YAxis domain={[0, 100]} hide />
                      <Tooltip contentStyle={{ borderRadius: '24px', border: 'none', backgroundColor: isDark ? '#1a1a1c' : '#fff', padding: '16px' }} />
              ...
              <div className="lg:col-span-4 bg-gradient-to-br from-[#6366F1] to-[#8B5CF6] text-white p-10 rounded-[3.5rem] shadow-2xl flex flex-col relative overflow-hidden">
                <div className="absolute top-[-10%] right-[-10%] opacity-20 rotate-12"><Waves size={400} /></div>
                <div className="flex justify-between items-start mb-10 relative z-10">
                  <div>
                    <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-white/60">Health Score</h4>
                    <p className="text-7xl font-black mt-2">{aiReport?.overallScore || "--"}</p>
                  </div>
                  <ShieldCheck size={28} className="text-white/40"/>
                </div>
                {aiReport ? (
                  <div className="space-y-6 flex-grow relative z-10 overflow-y-auto pr-2">
                    <div className="bg-white/10 p-5 rounded-[2rem] border border-white/20 backdrop-blur-md">
                      <p className="text-sm font-bold leading-relaxed italic">"{aiReport.summary}"</p>
                    </div>
                    <ul className="space-y-6">
                      {aiReport.actionPlan?.map((s, i) => (
                        <li key={i} className="flex gap-4 items-start">
                          <div className="shrink-0 bg-white text-indigo-600 h-8 w-8 rounded-xl flex items-center justify-center font-black text-xs">{i+1}</div>
                          <div className="flex flex-col">
                            <p className="font-black text-[10px] uppercase tracking-wider text-white leading-none">{typeof s === 'object' ? s.step : 'Task'}</p>
                            <p className="text-[11px] text-white/70 mt-2 font-bold leading-tight">{typeof s === 'object' ? s.description : s}</p>
                          </div>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="flex flex-col items-center justify-center flex-grow relative z-10">
                    ...
                    <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">Neural Link Syncing</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {/* FOOTER */}
        <footer className={`flex justify-between items-center p-6 px-12 rounded-[2.5rem] border text-[10px] font-black uppercase tracking-[0.2em] 
          ${isDark ? 'bg-black/40 border-white/5 text-white/20' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
