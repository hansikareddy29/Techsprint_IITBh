...
import axios from 'axios';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { Activity, Cpu, Battery, CheckCircle, AlertCircle, RefreshCw, AppWindow, Zap, ShieldCheck, Clock } from 'lucide-react';

const API_BASE = "http://localhost:8080/api";

export default function App() {
  const [deviceId, setDeviceId] = useState(localStorage.getItem('voltguard_device_id') || "");
  const [history, setHistory] = useState([]);
  const [stats, setStats] = useState({});
  const [aiReport, setAiReport] = useState(null);
  const [loading, setLoading] = useState(true);

  // 1. Device Identity & Privacy (Only shows the user's specific device)
  useEffect(() => {
    if (!deviceId) {
      axios.get(`${API_BASE}/logs/list-devices`)
        .then(res => {
          if (res.data.devices && res.data.devices.length > 0) {
            const myDevice = res.data.devices[0]; 
            setDeviceId(myDevice);
            localStorage.setItem('voltguard_device_id', myDevice);
          }
        })
        .catch(err => console.error("Privacy error: Identity check failed", err));
    }
  }, [deviceId]);

  // 2. Background Polling (30m Hardware / 1h AI)
  useEffect(() => {
    if (!deviceId) return;

    // TASK A: Fetch Hardware Metrics (30 min interval)
    const fetchHardware = async () => {
      try {
        ...
        setStats(sRes.data);
        setLoading(false);
        console.log("📊 Hardware metrics refreshed (30m interval)");
      } catch (e) {
        console.error("Hardware sync failed:", e.message);
      }
    };

    // TASK B: Fetch AI Diagnosis (1 hour interval)
    const fetchAI = async () => {
      try {
        const aiRes = await axios.get(`${API_BASE}/ai/diagnosis/${deviceId}`);
        setAiReport(aiRes.data);
        console.log("🤖 AI Diagnosis updated (1h interval)");
      } catch (err) {
        console.warn("AI Analysis is still processing or server is offline.");
      }
    };

    // Initial immediate calls to populate the UI on load
    fetchHardware();
    fetchAI();

    // SET LONG-TERM TIMERS
    const hardwareTimer = setInterval(fetchHardware, 1800000); // 30 Minutes
    const aiTimer = setInterval(fetchAI, 3600000);             // 1 Hour

    // CLEANUP: Kill heartbeats if tab is closed/device changes
    return () => {
  ...
  if (loading && !deviceId) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          <RefreshCw className="animate-spin mx-auto text-indigo-600 mb-4" size={48} />
          <h2 className="text-xl font-semibold text-slate-700">Identifying Device Node...</h2>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-black text-slate-800 tracking-tight">VoltGuard AI</h1>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Background Health Monitor</p>
              </div>
            </div>
            
            <div className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 rounded-full border border-indigo-100">
              <ShieldCheck size={18}/>
              <span className="text-xs font-bold uppercase">Private: {deviceId}</span>
            </div>
        </header>
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-indigo-50 flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-slate-400 uppercase mb-1 flex items-center gap-1">
                    <Clock size={12}/> Avg CPU
                  </div>
                  <div className="text-4xl font-black text-slate-800">{stats.avgCpu || 0}%</div>
                </div>
                <div className="text-indigo-400"><Cpu size={36}/></div>
            </div>
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-emerald-50 flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-slate-400 uppercase mb-1">Current Battery</div>
                  <div className="text-4xl font-black text-slate-800">{stats.lastBattery || 0}%</div>
                </div>
                <div className="text-emerald-400"><Battery size={36}/></div>
            </div>
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-red-50 flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-slate-400 uppercase mb-1">Total Alerts</div>
                  <div className="text-4xl font-black text-red-500">{stats.totalAlerts || 0}</div>
                </div>
                <div className="text-red-300"><AlertCircle size={36}/></div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="font-bold text-slate-700 flex items-center gap-2 text-sm uppercase tracking-wide">
                    <Zap size={18} className="text-yellow-500"/> Performance History (30m Interval)
                  </h3>
                </div>
                <div className="h-[380px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
            ...
            {/* AI DIAGNOSIS (Stable version) */}
            <div className="bg-indigo-600 text-white p-8 rounded-[2.5rem] shadow-2xl flex flex-col justify-between relative overflow-hidden">
                <div className="relative z-10">
                    <div className="flex justify-between items-start mb-6">
                      <div>
                          <div className="text-6xl font-black mb-1">{aiReport?.overallScore || "--"}</div>
                          <div className="text-[10px] uppercase font-bold opacity-70 tracking-widest flex items-center gap-2">
                             Health Score <Clock size={10}/> 1h
                          </div>
                      </div>
                      <div className="bg-white/20 p-3 rounded-2xl backdrop-blur-md">
                        <Activity size={24}/>
                    ...
                    <div className="bg-black/20 p-5 rounded-2xl mb-6 border border-white/10 min-h-[120px]">
                      <p className="text-sm font-medium leading-relaxed italic opacity-95">
                        "{aiReport?.summary || 'Generating hourly system health overview...'}"
                      </p>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div>
                          <div className="text-[10px] font-black uppercase opacity-60 mb-3 flex items-center gap-2">
                            <AppWindow size={14}/> Background Culprits
                          </div>
                          <div className="flex flex-wrap gap-2">
                              {aiReport?.culprits?.map((app, i) => (
                                  <span key={i} className="px-3 py-1 bg-white/20 rounded-lg text-xs font-bold border border-white/20">{app}</span>
                              )) || <span className="text-xs opacity-50 italic">Monitoring...</span>}
                          </div>
                        </div>

                        <div>
                          <div className="text-[10px] font-black uppercase opacity-60 mb-3">Long-term Action Plan</div>
                          <div className="space-y-3">
                              {aiReport?.actionPlan?.map((step, i) => (
                                  <div key={i} className="flex gap-3 text-sm items-start bg-white/5 p-2 rounded-xl border border-white/5">
                                      <CheckCircle className="mt-0.5 flex-shrink-0 text-emerald-400" size={16}/> 
                                      <span className="opacity-90">{step}</span>
                                  </div>
                              )) || <div className="text-xs opacity-50 italic">Preparing advice...</div>}
                          </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-white/10 relative z-10">
                    <div className="text-[10px] opacity-60 uppercase font-black mb-1">AI 24h Prediction</div>
                    <div className="text-sm font-medium text-indigo-100 italic">
                      {aiReport?.prediction || "Trend calculation starting..."}
                    </div>
                </div>
            </div>
