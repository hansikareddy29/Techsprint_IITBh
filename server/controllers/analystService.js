 ...
 * Analyzes the last 100 logs to create a persistent "Memory" for the AI.
 */
exports.generateLongTermTrend = async (deviceId) => {
    try {
        ...
        // Save this memory so the aiController can read it later
        await db.collection('device_summaries').doc(deviceId).set({
            deviceId,
